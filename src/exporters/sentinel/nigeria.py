...
from src.processors.nigeria import NigeriaProcessor, NigeriaProcessorNew
...
class NigeriaSentinelExporterNew(BaseSentinelExporter):

    dataset = "earth_engine_nigeria"

    # Ohotointerpretation with google satellite images from QGIS and google earth. Average dates between 2019 and 2020
    data_date = date(2020, 3, 28)

    def load_labels(self) -> pd.DataFrame:
        nigeria = self.data_folder / "processed" / NigeriaProcessorNew.dataset / "data.geojson"
        assert nigeria.exists(), "Nigeria processor must be run to load labels"
        return geopandas.read_file(nigeria)[["lat", "lon"]]
