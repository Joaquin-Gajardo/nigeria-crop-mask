...
from src.processors import NigeriaProcessor, NigeriaProcessorNew
from src.exporters import NigeriaSentinelExporter, NigeriaSentinelExporterNew
...
class NigeriaEngineerNew(BaseEngineer):

    sentinel_dataset = NigeriaSentinelExporterNew.dataset
    dataset = NigeriaProcessorNew.dataset

    @staticmethod
    def read_labels(data_folder: Path) -> pd.DataFrame:
        nigeria = data_folder / "processed" / NigeriaProcessorNew.dataset / "data.geojson"
        assert nigeria.exists(), "Nigeria processor must be run to load labels"
        return geopandas.read_file(nigeria)
